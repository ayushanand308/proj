...
const Signup = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if(PasswordsMatch(password,confirmPassword)){
      try {
        const response = await fetch('https://frustated.onrender.com/signup', {
          method: 'POST',
          body: JSON.stringify({
            email: email,
            password: password,
        ...
        if((json.message).indexOf("exists")===-1){
          navigate('/login');
        }
        setUserExists(true);
        ...
        <div class="p-8 flex-1">
          <div class="w-80 bg-white rounded-3xl mx-auto overflow-hidden shadow-xl">
            ...
            <div class="px-10 pt-4 pb-8 bg-white rounded-tr-4xl">
              <h1 class="text-2xl font-semibold text-gray-900">Get Started!</h1>
              <form class="mt-12" onSubmit={handleSubmit} action="" method="POST">
                <div class="relative">
                  <input
                    id="email"
                    name="email"
