...
import { faUser,faUserPlus,faMessage ,faImage,faRightFromBracket,faBars} from '@fortawesome/free-solid-svg-icons';
...
const Home = ({
  ...
  return (
    <div className="flex flex-col overflow-x-hidden  items-center justify-stretch min-h-screen py-4 bg-rose-100">
      <nav className="hidden sm:flex items-center justify-center fixed top-5 w-1/2 rounded-xl bg-rose-500 p-4">
        <div>
          {isLoggedIn ? (
            <button
      ...
      <nav className="sm:hidden flex items-center justify-between fixed top-0 w-full  bg-rose-500 p-4">
        <button
          onClick={toggleMenu}
